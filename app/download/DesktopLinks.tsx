...
export default function DesktopLinks() {
  return (
    <div className="py-5 mx-auto max-w-2xl lg:max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          .map((link: AppLink) => (
            <div key={link.id} className="flex flex-col items-center text-center px-3 py-5">
              ...
              {link.subLinks && link.subLinks.length > 0 ? (
                ...
                <Link
                  className="rounded-xl flex items-center justify-center bg-[#bf360c] gap-2 hover:bg-[#b86918] sm:text-base h-10 sm:h-10 px-4 sm:px-8 text-xs md:text-sm"
                  href={`${link.url}`}
                  role="button"
                  rel="noopener noreferrer"
                >
